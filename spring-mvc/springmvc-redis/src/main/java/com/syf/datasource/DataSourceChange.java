...
/*
* 自定义注解 add by syf 2019.5.1
  注解的定义：Java文件叫做Annotation，用@interface表示
  元注解：@interface上面按需要注解上一些东西，包括@Retention、@Target、@Document、@Inherited四种
  注解的保留策略: @Retention(RetentionPolicy.RUNTIME)  // 注解会在class字节码文件中存在，在运行时可以通过反射获取到
  注解的作用目标: @Target(ElementType.METHOD)// 方法
  注解可以被继承: @Inherited
 * */
@Inherited
...
/*
自定义注解(https://www.cnblogs.com/liangweiping/p/3837332.html)
注解的定义：Java文件叫做Annotation，用@interface表示
元注解：@interface上面按需要注解上一些东西，包括@Retention、@Target、@Document、@Inherited四种
注解的保留策略
